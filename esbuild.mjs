...
const { default: JBrowseReExports } =
  await import('@jbrowse/core/ReExports/list')
...
const config = {
  ...
  bundle: true,
  alias: { '@jbrowse/mobx-state-tree': 'mobx-state-tree' },
  globalName: 'JBrowsePluginTrackHubRegistry',
  metafile: true,
  plugins: [globalExternals(createGlobalMap(globals)), rebuildLogPlugin],
  ...(isWatch
    ? { outfile: 'dist/out.js' }
    : {
        outfile: 'dist/jbrowse-plugin-trackhub-registry.umd.production.min.js',
        sourcemap: true,
        minify: true,
      }),
}
